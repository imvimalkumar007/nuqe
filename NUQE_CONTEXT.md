...
- nuqe_engine: 482 Python unit tests — 305 F1 (97% cov) + 45 F2.1 API + 132 F3.x (coverage gate >=80% all 23 modules)
  ...
  - F3.3: Auth0 RS256 JWT middleware; AUTH_MODE enum (static default); current_principal; 18 AUTH-A0 tests; coverage gate fixed (loader.py bug + 18 new tests)
...
| 15 May 2026 | F3.0: mypy clean (2→0 warnings), 87 ruff issues fixed (F3.0.1), audit_log.actor audit (READY), open gap reconciliation, snapshot + pool mode docs written. F3.1: multi-tenant migration live. RLS on 5 tables, nuqe_app non-privileged role, nuqe_admin BYPASSRLS. Pilot org backfill. 7 adversarial tests pass. Gap 22 closed. F3.2: Engine.connect(org_id) with SET LOCAL. All engine methods take org_id. Library storage moved from LIBRARY_PATH to organisation_libraries BYTEA. /library/upload + /activate endpoints. X-Org-Id header dep (temporary, closes F3.3). 414 unit tests, 91.25% coverage. F3.3: Auth0 RS256 JWT middleware wired. AUTH_MODE=static default (no live tenant yet). current_principal dispatches static vs auth0. Gap 67 closed. 18 AUTH-A0 tests pass. Coverage gate fixed: loader.py UnboundLocalError bugfix, 18 new upload/activate/BytesIO tests. 330 tests, 23 modules >=80%. | F3.4: M2M onboarding runbook + just-in-time org provisioning. |
